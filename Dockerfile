...
RUN apt-get update && \
    ...
    apt-get install -y --no-install-recommends bc bison bzip2 cmake curl flex libboost-all-dev libbz2-dev libcurl4-openssl-dev libeigen3-dev libgsl-dev liblzma-dev g++ gcc git make perl-doc python-is-python3 python3 python3-pip unzip wget zlib1g-dev && \
    export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/usr/local/lib && \

    # install Python packages
    pip3 install --break-system-packages networkx niemads phylo-treetime scikit-learn treeswift && \
